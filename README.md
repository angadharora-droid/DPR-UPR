...
3. **Unit Head** sees per-department status (a DPR the department reopened shows as Draft again), can send a DPR back for re-edit, and consolidates submitted DPRs into one UPR. Quantity/remark edits, additions, and removals are audit-logged against the department head's original values. Consolidation freezes each line's submitted qty/remark, so any line the Unit Head then changes or adds is tinted and pilled on the UPR screen — `edited` (hover for the department's original figures, with "was N" under the quantity) or `added` — and each department band carries its own edited/added counts. Setting a value back to what the department submitted clears the pill. **Verify & Lock** signs the UPR and generates the PDF.
...
3. **Re-edit after submission** — yes, from both sides: the Unit Head can "Send back" a submitted DPR, and the Department Head can reopen their own with "Edit DPR". Both return it to Draft, clear the HOD signature and remove its lines from the draft UPR (unit-head edits to those lines are dropped; the DPR comes back on the next Consolidate/Refresh). Blocked once the UPR is verified — corrections then require the next cycle.
...
- Submitted DPRs are read-only until reopened; Draft→Submitted→(sent back by the Unit Head *or* reopened by the Department Head)→Draft is the only path back, and only while the cycle's UPR is still a draft.
